   ...
   while ((currentByte = get_next_byte(get_next_byte_argument)) != EOF)  {
      ...
      switch (currentByte) {
          ...
          case '\n':
             ...
             break;
                case '#':
                  if  ((num_words == 0 && num_command == 0)  || space_seen || prevNecessarySlash ||  ((num_words > 0) && ((strcmp(words[num_words -1],"||") == 0) || (strcmp(words[num_words -1],")") == 0) || (strcmp(words[num_words -1],"&&") == 0) || (strcmp(words[num_words -1],"|") == 0))))
                  {
                   if (num_command > 0) {
                     if (words_array_size == num_words)
                         {
